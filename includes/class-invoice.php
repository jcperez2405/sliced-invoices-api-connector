...
class Invoice extends WP_REST_Controller {
	...
	public function get_items( $request ) {
	    ...
	    foreach ( $quotes as $quote ) {
	    	$data 			= $this->prepare_item_for_response( $quote, $request );
	...
	public function get_item( $request ) {
        $q = get_post( $request->get_param('id') );

        //return get_post_meta( $request->get_param('id') );

		if ( ! empty( $q ) && 'sliced_invoice' == get_post_type( $request->get_param('id') ) ) {
			return $this->prepare_item_for_response( $q, $request );
		}

		return new WP_Error( 'rest_invoice_invalid_id', __( 'Invalid invoice ID.', 'sliced-invoices-api-connector' ), array( 'status' => 404 ) );
	}
    ...
    public function create_item_args() {
        ...
        $args['items'] = array(
            'description'  => esc_html__( 'An array containing the items for the invoice. Name, Quantity and Price.', 'sliced-invoices-api-connector' ),
            'type'         => 'array',
            'required'     => true
        );

        $args['action'] = array(
            'description'   => esc_html__( 'Action to execute with the invoice. Valid values are DRAFT | SEND.', 'sliced-invoices-api-connector' ),
            'type'          => 'string',
            'required'      => true,
        ...
        $args['discount'] = array(
            'description'  => esc_html__( 'The discount applied to the invoice. Must be integer.', 'sliced-invoices-api-connector' ),
            'type'         => 'integer',
        );
	...
	public function update_item_args() {
	    ...
	    $args['id'] = array(
	        'description'  => esc_html__( 'The Invoice ID', 'sliced-invoices-api-connector' ),
	        'type'         => 'integer',
	        'required'     => true,
	    );
        ...
        foreach ($request['items'] as $item) {
            foreach ($keys as $k) {
                if ( ! array_key_exists( $k, $item ) ) {
                    return new WP_Error(
                        'rest_invoice_missing_param_key',
	...
	public function create_item( $request ) {
        ...
        $term = term_exists( $this->get_term( $params->get_param('action') ), 'invoice_status');

        if ( empty( $term ) ) {
            return new WP_Error(
                'rest_invoice_invalid_term',
        ...
        wp_set_post_terms( $id, [$term['term_id']], 'invoice_status' );

        if ( $params->get_param('action') == 'Sent' ) {
            $n = new Sliced_Notifications();
            $n->send_the_invoice( $id );
        }
	 
		return new WP_REST_Response( $this->prepare_item_for_response( get_post( $id ), $request ) , 201 );
	}
	...
	public function update_item( $request ) {
        ...
        if ( empty( $term ) ) {
            return new WP_Error(
                'rest_invoice_invalid_term',
                __( 'The selected status doesnt exists for invoices.', 'sliced-invoices-api-connector' ),
                array(
                    'status' => 400
                )
            );
        ...
        wp_set_post_terms( $params->get_param('id'), [$term['term_id']], 'invoice_status' );

		if ( $params->get_param('action') == 'Sent' ) {
            $n = new Sliced_Notifications();
            $n->send_the_invoice( $params->get_param('id') );
        }

        return new WP_REST_Response( $this->prepare_item_for_response( get_post( $params->get_param('id') ), $request ) , 202 );
	}
	...
	private function prepare_item( $request ) {
        $s = new Sliced_Invoice();

        return [
            'post_title'        => "INV-{$s->get_next_invoice_number()}",
            'post_type'         => 'sliced_invoice',
            'post_status'       => 'publish',
            'comment_status'    => 'closed',
            'ping_status'       => 'closed',
            'meta_input'        => [
                '_sliced_invoice_prefix'    => "INV-",
                '_sliced_invoice_number'    => $s->get_next_invoice_number(),
                '_sliced_client'            => $this->getOrCreateCustomer( $request['customer_email'] ),
                '_sliced_items'             => $request['items'],
                '_sliced_invoice_created'   => current_time( 'timestamp' ),
                'sliced_invoice_discount'   => $request['discount'],
                '_sliced_currency_symbol'   => '$',
                '_sliced_currency'          => 'default',
	...
	public function prepare_item_for_response( $item, $request ) {
        ...
        $s = get_the_terms( $item->ID, 'invoice_status' );

	    return rest_ensure_response([
	    	'id' 			=> $item->ID,
	    	'number'		=> $m['_sliced_invoice_prefix'][0] . $m['_sliced_invoice_number'][0],
	    	'status' 		=> $s[0]->name,
	    	'customer'		=> [
	    		'name'	=> $c->nickname,
	    		'email'	=> $c->user_email
	    	...
	    	'created_at'	=> $m['_sliced_invoice_created'][0]
	    ]);
	}
	...
	public function get_item_schema() {
		return [
	        ...
	        'properties'	=> array(
	            ...
	            'number'	=> array(
	                'description'  => esc_html__( 'The number of the invoice.', 'sliced-invoices-api-connector' ),
	                'type'         => 'string',
	            ),
	            'status'	=> array(
	                'description'  => esc_html__( 'The status of the invoice.', 'sliced-invoices-api-connector' ),
	                'type'         => 'string',
	            ),
		    	'wc-order'	=> array(
	                'description'  => esc_html__( 'The id of the associate woocommerce order, if any.', 'sliced-invoices-api-connector' ),
	                'type'         => 'integer',
	            ),
		    	'items'	=> array(
		    	...
		    	'created_at'	=> array(
	                'description'  => esc_html__( 'The date when the invoice was created.', 'sliced-invoices-api-connector' ),
	                'type'         => 'string',
	            )
	        )
	    ];
